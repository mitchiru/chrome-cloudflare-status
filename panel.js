document.addEventListener('DOMContentLoaded', function () {
    ...
    if (tableBody) {
        ...
        chrome.devtools.network.onRequestFinished.addListener(request => {
            ...
            const fileSize = request.response.content.size; // File size in bytes
			...
			const cellFileSize = row.insertCell(5); // New cell for file size
			cellFileSize.style.textAlign = 'right';
            ...
            cellFileSize.textContent = `${Math.ceil(fileSize / 1024)}`; // Display file size in KB
        });
    }
});
